...
struct arena{
	...
	//large 为true时，表示页框数，不然表示空闲mem_block数量
	uint32_t cnt;
	bool large;
};
//内核的内存块描述符数组
...
void* sys_malloc(uint32_t size){
	...
	if(size > 1024){
		...
		//如果块描述符，描述的空闲块链表中已经没有，则新分配arena
...
/* 得到虚拟地址vaddr对应的pte指针  因为在分页模式下，程序访问页表也要用虚拟地址，所以这里
 *根据给定的虚拟地址，得到访问该虚拟地址对应页表的虚拟地址
 * */
...
void pfree(uint32_t pg_phy_addr){
	struct pool* mem_pool;
	uint32_t bit_idx = 0;
	if(pg_phy_addr >= user_pool.phy_addr_start){//用户物理地址
		mem_pool = &user_pool;
...
void mfree_page(enum pool_flags pf, void* _vaddr, uint32_t pg_cnt){
	...
	if(pg_phy_addr >= user_pool.phy_addr_start){
		...
		while(page_cnt < pg_cnt){
			...
			ASSERT((pg_phy_addr % PG_SIZE) == 0 && pg_phy_addr < user_pool.phy_addr_start && pg_phy_addr >= kernel_pool.phy_addr_start);
			pfree(pg_phy_addr);
			page_table_pte_remove(vaddr);
			page_cnt++;
		}
...
//将vaddr 与 对应内存池中的物理地址相关联(该函数仅支持一页内存的分配)
void* get_a_page(enum pool_flags pf, uint32_t vaddr){
	...
	//内核线程
		...
		PANIC("get_a_page: invaild get a page,kernel alloc userspace or user alloc kernelspace");
	}
